...
class PageBrowsingViewHelper extends AbstractTagBasedViewHelper
{
    ...
    public function render()
    {
        ...
        for ($a = $minPage; $a <= $maxPage; $a++) {
            ...
            if ($a === $currentPage) {
                $content .= '<li class="tx-indexedsearch-browselist-currentPage page-item active"><strong>' . $label . '</strong></li>';
            } else {
        ...
        if (!$this->tag->hasAttribute('class')) {
            $this->tag->addAttribute('class', 'tx-indexedsearch-browsebox pagination justify-content-center');
        }
    ...
    protected function makecurrentPageSelector_link($str, $p, $freeIndexUid)
    {
        ...
        return '<a href="#" class="page-link rounded-0" onclick="' . htmlspecialchars($onclick) . '">' . htmlspecialchars($str) . '</a>';
    }
}